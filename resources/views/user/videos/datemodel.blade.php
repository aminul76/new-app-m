...
@section('style')
    ...
    <style>
        ...
        .active-date::after {
            content: "✓";
            /* Optional: add a checkmark or any other icon to indicate active */
            position: absolute;
            bottom: -10px;
            /* Position it below the circle */
            right: -10px;
            font-size: 16px;
            color: #fff;
        }
    ...
    $message = "Time's up";
        ...
        <div class="day-header-container">
            <div class="day-header">
                @foreach ($dates as $date)
                    <div id="{{ $date['isselectdate'] ? 'current-date' : '' }}"
                        class="{{ $date['isselectdate'] ? 'active-date' : '' }}">
        ...
        <div class="routine-card">
            ...
            <div class="exam-cards">
                {{-- @forelse ($videos as $modelTest)
            <div class="exam-card" data-title="{{ $modelTest->title }}" data-description="{{ $modelTest->m_description }}" data-times="Start Time: {{ \Carbon\Carbon::parse($modelTest->start_date)->format('g:i A') }} - End Time: {{ \Carbon\Carbon::parse($modelTest->end_date)->format('g:i A') }}" >
                ...
                @foreach ($videos as $video)
                    <div class="container" data-title="{{ $video->title }}" data-description="{{ $video->m_description }}"
                        data-times="Start Time: {{ \Carbon\Carbon::parse($video->start_date)->format('g:i A') }} - End Time: {{ \Carbon\Carbon::parse($video->end_date)->format('g:i A') }}">
                        <a href="#" class="exam-card">
                        ...
                        <div class="button-group" data-title="{{ $video->title }}" data-description="{{ $video->m_description }}"
                            data-times="Start Time: {{ \Carbon\Carbon::parse($video->start_date)->format('g:i A') }} - End Time: {{ \Carbon\Carbon::parse($video->end_date)->format('g:i A') }}">
                            <a href="#" class="action-button">Start Exam</a>
                            <a href="#" class="action-button">Lecture Sheet</a>
                        </div>
